...
class AutoScaler : IDisposable, IInitializable, ITransport
{
    ...
    {
        _logger.Info("Initializing auto-scaler - will add up to {count} workers", _maximumNumberOfWorkers);
        _task.Start();
    }
    ...
    {
        _logger.Debug("Auto-scale to {count} workers", newNumberOfWorkers);
